...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.action === 'reorder') {
      ...
      const allMembers = (await db.select().from(teamMembers).orderBy(asc(teamMembers.sortOrder)).all()) || [];
    ...
    const updated = await db.select().from(teamMembers).where(eq(teamMembers.id, resolvedParams.id)).get();
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const resolvedParams = await params;
    await db.delete(teamMembers).where(eq(teamMembers.id, resolvedParams.id)).run();
    return NextResponse.json({ success: true });
  } catch (error) {
